...
class Request
{
     ...
     * Get uploaded file
     *
     * Always returns array as client can upload multiple files
     *
     * @param string $name
     * @return UploadedFile[]
     */
    public function getFile($name)
    {
        if (isset($this->files[$name])) {
            ...
            if (is_array($data['tmp_name'])) {
                $file  = [];
                $total = count($data['tmp_name']);
                $get   = function ($n, $i) use ($data) {
                    return isset($data[$n][$i]) ? $data[$n][$i] : '';
                };
                for ($i = 0; $i < $total; $i++) {
                    $buffer = [
                        'name'     => $get('name', $i),
                        'tmp_name' => $get('tmp_name', $i),
                        'type'     => $get('type', $i),
                        'error'    => $get('error', $i),
                    ];
                    $file[] = new UploadedFile($buffer);
                }
            } else {
